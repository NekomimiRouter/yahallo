...
// MessageText:
//
// When trying to update a password, this status indicates that some password
// update rule has been violated. For example, the password may not meet length
// criteria.
//
...
MessageId's 0xa2a1 - 0xa300 (inclusive) are for File System Filters Supported
Features specific messages.
...
// MessageText:
//
// The cloud sync engine failed to perform the operation due to low system
// resources.
//
...
// MessageText:
//
// The cloud sync engine failed to perform the operation due to network being
// unavailable.
//
...
// Partial data populated upto the size of the buffer. Caller needs to provide
...
// MessageText:
//
// The TPM hardware device reported a failure during its internal self test. Try
// restarting the computer to resolve the problem. If the problem continues, you
// might need to replace your TPM hardware or motherboard.
//
...
// paritions. This is an invalid disk layout.
//
...
// MessageText:
//
// The specified parition overlaps an EBR (the first track of an extended
...
// MessageText:
//
// The System Integrity policy is either not signed or is signed by a
// non-trusted signer.
//
...
#define __NTSTATUS_FROM_WIN32(x)                                               \
  ((NTSTATUS)(x) <= 0 ? ((NTSTATUS)(x))                                        \
                      : ((NTSTATUS)(                                           \
                            ((x)&0x0000FFFF) | (FACILITY_NTWIN32 << 16) |      \
